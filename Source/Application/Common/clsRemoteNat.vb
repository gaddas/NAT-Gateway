...
Public Interface remoteObjectNAT

    Function GetConfig() As String
    Sub SetConfig(ByVal config As String)

    Function GetInterfaces() As Dictionary(Of String, String)
    Function GetGateway() As Net.IPAddress

    Function Test(ByVal x As Integer) As Integer

End Interface

Public Class remoteFactoryNAT
    Inherits MarshalByRefObject

    Public Const defaultPort As Integer = 7001
    Public Const defaultURI As String = "nat.rem"

    Private obj As remoteObjectNAT = Nothing

    Public Sub New(ByVal initObj As remoteObjectNAT)
        obj = initObj
    End Sub
    ...
    Public Function Login(ByVal user As String, ByVal pass As String) As remoteObjectNAT
        ...
        While Not r.EndOfStream
            ...
            If s.Remove(s.IndexOf("|")).ToLower = user.ToLower AndAlso s.Substring(s.IndexOf("|") + 1) = pass Then
                ...
                r.Close()
                f.Close()
                Return CType(obj, remoteObjectNAT)
            End If
        End While
