...
// BEGIN_PTEX_FILTER
        //
        // Initialize Filter Resources On This Adapter
        //
...
--*/
{
	...
	// Is this our injected packet?
	if( (((PNPROT_SEND_PACKET_RSVD)&((Packet)->ProtocolReserved[0]))->pIrp) != NULL) {
		netgSendToMiniportComplete(ProtocolBindingContext, Packet, Status);
		return;
	}	
...
// BEGIN_PTEX_FILTER
        ...
        //
        if( RcvFltAction & ACTION_BLOCK_PACKET )
        {
...
// BEGIN_PTEX_FILTER
   //
   // Call Receive Packet Filter
   //
   RcvFltAction = FltFilterReceivePacket( pAdapt, Packet );
